...
def preserve_patches_and_fill_rest(img, patch_size=(4, 16, 16), fill_with_noise=0):
    """
    Preserve a portion of the input image with patches and fill the rest with 0 or noise.

    Args:
    - img (torch.Tensor): Input image tensor.
    - patch_size (tuple): Size of the patch in (height, width, depth).
    - fill_with_noise (bool): Whether to fill the rest with noise or 0.
    - mode (int): Determines how the next_token is computed. 

    Returns:
    - torch.Tensor: Image tensor with preserved patches and filled rest.
    - torch.Tensor: Next token's true value.
    """
    # Get image shape
    img_shape = img.shape
    h, w, d = img_shape
    ...
    # Tokenize and reshape as L * p1 * p2 * p3
    img_token = rearrange(
        img, 
        '(h p1) (w p2) (d p3) -> (h w d) p1 p2 p3', 
        p1=patch_h, p2=patch_w, p3=patch_d
    )

    # Randomly determine the number of patches to preserve
    num_patches_to_preserve = np.random.randint(1, total_patches)


    next_token = img_token[num_patches_to_preserve]
  
    next_token_all = img_token[num_patches_to_preserve:] / len(img_token[num_patches_to_preserve:])

    # Initialize the generated token array
    generate_token = np.zeros_like(img_token)
    generate_token[:num_patches_to_preserve] = img_token[:num_patches_to_preserve]

    # Fill the rest of the tokens with noise or zeros
    if fill_with_noise == 1:
        generate_token[num_patches_to_preserve:] = np.random.normal(
            0, 1, generate_token[num_patches_to_preserve:].shape
        )
    else:
        generate_token[num_patches_to_preserve:] = 0

    # Reshape back to the original image shape
    img_token = rearrange(
        img_token, 
        '(h w d) p1 p2 p3 -> (h p1) (w p2) (d p3)', 
        h=h // patch_h, w=w // patch_w, d=d // patch_d
    )
    generate_token = rearrange(
        generate_token, 
        '(h w d) p1 p2 p3 -> (h p1) (w p2) (d p3)', 
        h=h // patch_h, w=w // patch_w, d=d // patch_d
    )

    return generate_token, next_token, next_token_all

def cover_image_with_patches(img, patch_size=(16, 16, 16), mask_ratio=0.1):
    """
	...
	def __getitem__(self, index):
		...
		if self.cfg.trainer.contranstive:
			...
			if random.random() > 0.5:
				imgs1, _, _, _ = self.apply_perturbations(imgs, None, mode=self.per_mode)
			imgs1, next_token, next_token_all = preserve_patches_and_fill_rest(imgs1, patch_size=(4, 16, 16), fill_with_noise=self.args.fill_nose)
			# gt_imgs = imgs[:, self.sub_padding[-1]:-self.sub_padding[-1], self.sub_padding[-1]:-self.sub_padding[-1]]
			imgs1 = imgs1[np.newaxis, ...]
			gt_imgs = gt_imgs[np.newaxis, ...]
			next_token = next_token[np.newaxis, ...]
			imgs1 = np.ascontiguousarray(imgs1, dtype=np.float32)
			gt_imgs = np.ascontiguousarray(gt_imgs, dtype=np.float32)
			return imgs1, gt_imgs, next_token, next_token_all
